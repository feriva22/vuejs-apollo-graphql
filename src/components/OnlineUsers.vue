...
<script>
  ...
  const SUBSCRIPTION_ONLINE_USERS = gql`
    subscription getOnlineUsers{
      online_users(order_by: {user : {name: desc}}){
        id
        user {
          name
        }
  ...
  export default {
    ...
    mounted(){
      ...
      setInterval(function() {
       this.$apollo
         .mutate({
           mutation: UPDATE_LASTSEEN_MUTATION, //update current status of login user
           variables: {
    ...
    apollo: {
      //Subscription
      $subscribe: { //subscribe for event every change on state user
